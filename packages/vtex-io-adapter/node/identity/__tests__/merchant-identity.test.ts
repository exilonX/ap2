...
import { verifyCartMandate } from '../../core'
import type { CartData } from '../../core'
...
describe('MerchantIdentity', () => {
  ...
  it('multiple getDID calls do not regenerate keys (idempotent)', async () => {
    ...
    const stored1 = fake.peek<{ publicKeyHex: string }>(
      'acg-identity',
      'merchant-did'
    )
    ...
    const stored2 = fake.peek<{ publicKeyHex: string }>(
      'acg-identity',
      'merchant-did'
    )
  ...
  it('uses keys already present in the KeyStore', async () => {
    ...
    const seeded = fake.peek<{ publicKeyHex: string }>(
      'acg-identity',
      'merchant-did'
    )
