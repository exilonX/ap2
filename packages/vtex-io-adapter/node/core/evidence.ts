...
// The real v0.2-canonical shape lives in `./ap2/payment-mandate.ts`.
...
export function extractEvidenceBundle(
  ...
  if (
    ...
    typeof mandate.merchant_authorization !== 'string'
  ) {
    throw new Error(
      'extractEvidenceBundle: malformed mandate (missing merchant_authorization JWT)'
    )
  }
  ...
  if (parts.length !== 3) {
    throw new Error(
      'extractEvidenceBundle: malformed JWT in merchant_authorization'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `extractEvidenceBundle: failed to decode JWT payload (${
  ...
  if (!payload.cart_hash || typeof payload.cart_hash !== 'string') {
    throw new Error(
      'extractEvidenceBundle: JWT payload missing cart_hash claim'
    )
  }
  ...
  if (!mandate.contents.merchant_name) {
    throw new Error(
      'extractEvidenceBundle: mandate.contents.merchant_name is missing'
    )
  }
