...
export async function createPaymentReceipt(
  ...
  const allChecksPassed = Object.values(input.verification_checks).every(
    Boolean
  )

  const approvalStatus: 'approved' | 'rejected' = allChecksPassed
    ? 'approved'
    : 'rejected'
  ...
  const contents: PaymentReceiptContents = {
    ...
    rejection_reason:
      approvalStatus === 'rejected' ? input.rejection_reason : undefined,
...
export async function verifyPaymentReceipt(
  ...
  if (
    !receipt?.network_authorization ||
    typeof receipt.network_authorization !== 'string'
  ) {
    return {
      valid: false,
      checks: {
