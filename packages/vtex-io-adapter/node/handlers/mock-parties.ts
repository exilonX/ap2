...
import { verifyPaymentMandate, verifyPaymentReceipt } from '../core'
...
export async function getPaymentMandate(ctx: Context): Promise<void> {
  ...
  try {
    pm = await ctx.clients.vbase.getJSON<PaymentMandate>(
      PAYMENT_MANDATE_BUCKET,
      id,
      true
    )
  ...
  const { cpDomain } = buildDIDDomains(ctx)
  const baseUrl = `https://${
    ctx.vtex.workspace === 'master' ? '' : `${ctx.vtex.workspace}--`
  }${ctx.vtex.account}.myvtex.com`
...
export async function getPaymentReceipt(ctx: Context): Promise<void> {
  ...
  try {
    receipt = await ctx.clients.vbase.getJSON<PaymentReceipt>(
      PAYMENT_RECEIPT_BUCKET,
      id,
      true
    )
