...
export async function proposeDeal(ctx: Context) {
  try {
    ...
    if (cart.total < freeShippingThreshold && cart.total >= freeShippingThreshold * 0.6) {
    ...
    if (cart.items.length === 1) {
      deals.push({
        type: 'bundle',
        message: 'Complete your look! Add a matching accessory and get 20% off both items.',
        discount: 0.2,
    ...
    if (cart.total >= 300) {
      deals.push({
        type: 'tier_discount',
        message: 'Great cart! You qualify for our premium discount - 5% off automatically applied at checkout.',
        discount: 0.05,
