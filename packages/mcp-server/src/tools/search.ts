...
export function registerSearchTools(server: McpServer, client: VtexClient) {
  ...
  const browseProductsTool = server.tool(
    ...
    [
      ...
      'AND no gender signal is available (neither explicit in the current message nor established',
      'earlier in this conversation), you MUST NOT call this tool yet. Instead, ask the user',
      '"Pentru bărbați sau damă?" first.',
      '',
      'Gender signals that DO unblock the search (no need to ask):',
      '  - Conversation memory: if the user already specified a gender earlier in this session',
      '    (e.g. previously answered "Bărbați" / "Damă" to your clarification, or said',
      '    "vreau o cămașă pentru bărbați"), CARRY THAT GENDER FORWARD. Do not re-ask on every',
      '    apparel turn — once is enough.',
      ...
      '',
      'Color, size, material, occasion are NOT gender signals — but conversation history IS. Always',
      'check whether the gender was established earlier before asking again.',
      '',
